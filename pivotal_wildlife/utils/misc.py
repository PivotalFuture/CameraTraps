...
def process_video(
    ...
    assert (
        source_video_info.total_frames is not None
        and source_video_info.total_frames > 0
    ), f"Video file {source_path} is empty."
    ...
    with VideoSink(
        target_path=target_path, video_info=source_video_info, codec=codec
    ) as sink:
